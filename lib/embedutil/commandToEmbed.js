...
const getColor = require('../colors/get');
...
module.exports = (str) => {
	...
	if (parsedArgs.color || parsedArgs.colour) {
		...
		color = getColor(unparsedColor.trim().toLowerCase()) || parse(unparsedColor);
	...
	const embed = {
		...
		description: typeof parsedArgs.description === 'string' && parsedArgs.description.replace(/\\n/g, '\n'),
		fields: [],
		image: {
